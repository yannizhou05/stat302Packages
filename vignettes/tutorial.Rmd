...
cv_training <- vector()
for (i in 1:10) {
  class <- my_knn_cv(select(gapminder, lifeExp, gdpPercap),gapminder$continent,i,5)
  cv_record[i] = class$cv_error
  cv_training[i] = sum(gapminder$continent != class$class) / length(gapminder$continent)
}
```
